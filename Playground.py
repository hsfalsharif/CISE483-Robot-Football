...
class Playground:
	...
	def update(self):
		...
		for i in self.robotYellow:
			self.obsticals.append([i.position.x,i.position.y,0.0875*2])
	...
	def render_info(self):
		...
		#imgui.set_next_window_position(field_widget_postion[0],field_widget_postion[1])
		...
		for i in self.robotYellow:
			if imgui.tree_node("Robot{0}".format(i.robotID)):
				...
				imgui.text("current_path_segmen :{0}".format(i.current_path_segment))
				imgui.text("target_postion  :({0},{1})".format(i.target_postion.x,i.target_postion.y))
		...
		for i in self.robotYellow:
			if imgui.tree_node("Robot{0} ".format(i.robotID)):
				...
				if imgui.tree_node("obsticals"):
					for j in i.obsticals:
						imgui.bullet_text("{0}".format(j))
					imgui.tree_pop()
				imgui.text("max_velocity  :{0}".format(i.max_velocity))
				imgui.text("current_path_segmen :{0}".format(i.current_path_segment))
	...
	def render_map(self):
		...
		widget_margin = 25
		pixel_to_meter = 50
		field_widget_postion = [0,0]
		field_widget_size = [(width+(margine+refere_margine)*2)*pixel_to_meter+widget_margin*2,(lenght+(margine+refere_margine)*2)*pixel_to_meter+widget_margin*2]
		imgui.set_next_window_size(field_widget_size[0],field_widget_size[1])
		...
		x1,y1 = x0 + ((margine+refere_margine)*2+width)*pixel_to_meter, y0 
		x2,y2 = x0 , y0 + ((margine+refere_margine)*2+lenght)*pixel_to_meter
		x3,y3 = x1, y2
		...
		x4,y4 = x0 + pixel_to_meter*(refere_margine + margine), y0 + pixel_to_meter*(refere_margine + margine) 
		x5,y5 = x1 - pixel_to_meter*(refere_margine + margine), y1 + pixel_to_meter*(refere_margine + margine) 
		x6,y6 = x2 + pixel_to_meter*(refere_margine + margine), y2 - pixel_to_meter*(refere_margine + margine) 
		x7,y7 = x3 - pixel_to_meter*(refere_margine + margine), y3 - pixel_to_meter*(refere_margine + margine) 
		...
		# draw goalia 
		x8,y8   = xc - (penality_w/2)*pixel_to_meter,y4
		x9,y9   = xc + (penality_w/2)*pixel_to_meter,y4
		x10,y10 = x8  , y4 + (penality_d)*pixel_to_meter
		x11,y11 = x9 , y10
		x12,y12 = x8  , y6
		x13,y13 = x9  , y6
		x14,y14 = x10 , y6 - (penality_d)*pixel_to_meter
		x15,y15 = x11 , y14
		...
		if imgui.is_mouse_hovering_rect(xo,yo,xo+imgui.get_window_width(),yo+imgui.get_window_height()):
			imgui.set_tooltip("({0},{1})".format((imgui.get_mouse_pos()[0]-xc)/pixel_to_meter,(imgui.get_mouse_pos()[1]-yc)/pixel_to_meter))
	...
	def gui_field_projection(self,startx,starty,x,y):
		return [starty +6 - y,startx+ 8 - x]
